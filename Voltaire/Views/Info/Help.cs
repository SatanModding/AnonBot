    ...
    public static class Help
    {
        ...
        public static Tuple<string, Embed> Response(ShardedCommandContext context)
        {
            ...
            embed.AddField("send_dm {user name} {message}", "Sends an anonymous message to the specified user." +
                $"\nex: `send_dm @AnonBot The right man in the wrong place can make all the difference in the world.`");
            embed.AddField("send_server \"{server name}\" {channel name} {message}", "This command is only needed if you belong to " +
                "multiple servers that have AnonBot installed. It allows you to specify which server you are sending to." +
            ...
            embed.AddField("!volt admin", "(server admin only - callable from server channel) Get a list of admin commands, including details on AnonBot Pro.");
